...
# app.py (partial - update your existing app.py)
from flask import Flask, request, jsonify, render_template
from crop_yield import CropYieldPredictor, create_folder_structure
import os
...
# Make sure your models are in 'crop-yield-models' and data in 'crop-yield-data'
...
def predict():
    ...
    crop = data.get('crop')
    model = data.get('model', 'stacking')  # Default to stacking
    
    if not district or not crop:
        return jsonify({'error': 'District and crop are required'}), 400
    
    result = predictor.predict(district, crop, model_name=model)
...
def get_districts():
    ...
    return jsonify({'districts': predictor.get_district_list()})

@app.route('/api/crops', methods=['GET'])
def get_crops():
    """Get list of all crops"""
    return jsonify({'crops': predictor.get_crop_list()})

@app.route('/api/record/<district>/<crop>', methods=['GET'])
def get_record(district, crop):
    """Get the actual record for a district-crop combination"""
    record = predictor.get_district_crop_data(district, crop)
    if record:
...
@app.route('/api/compare/<district>/<crop>', methods=['GET'])
def compare_models(district, crop):
    """Compare predictions from all models"""
    result = predictor.predict_with_all_models(district, crop)
